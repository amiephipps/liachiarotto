...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <header class="entry-header">
        ...
        <?php elseif ( is_singular() ) :  ?>
            ...
            <?php if ( function_exists('yoast_breadcrumb') ) : ?>
                <?php // breadcrumbs must be turned on in Yoast's settings ?>
    ...
    <section class="entry-content">
        ...
        <?php if ( is_page() ) : // include the page-links if it is a page ?>
            <?php wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( 'Pages:', 'master_theme' ) . '</span>', 'after' => '</div>' ) ); ?>
        <?php endif; ?>
        ...
        <footer class="entry-meta">
            <div class="comments-link">
                <?php comments_popup_link( 
                     __( 'Leave a comment', 'master_theme' ), 
                     __( '1 comment', 'master_theme' ), 
                     __( '% comments', 'master_theme' ) ); 
                ?>
            </div>
        </footer> <!-- entry-meta -->
